...
contract("EqiFI", function (accounts) {
    ...
    const vestingDuration = aMonth * 9;
    ...
    beforeEach(async function () {
        ...
        eqiFi = await EqiFi.new(identity.address, rewardToken.address, startDate, ether("3000"));
        EF = eqiFi.address;
        await rewardToken.transfer(EF, ether("3000"));
    });
    ...
    describe("EqiFI Distribution Initializing Phase Test Cases", function () {
        ...
        it("should initialize vesting with correct duration", async () => {
            (await eqiFi.VESTING_DURATION()).should.be.bignumber.equal(vestingDuration.toString());
        });
        ...
        it("shouldn\'t initialize vesting with empty signer address", async () => {
            await expectRevert(
                EqiFi.new(constants.ZERO_ADDRESS, rewardToken.address, startDate, ether("3000")),
        ...
        it("shouldn\'t initialize vesting with empty reward token address", async () => {
            await expectRevert(
                EqiFi.new(identity.address, constants.ZERO_ADDRESS, startDate, ether("3000")),
        ...
        it("shouldn\'t initialize vesting with empty tge timestamp", async () => {
            await expectRevert(
                EqiFi.new(identity.address, rewardToken.address, 0, ether("3000")),
        ...
        it("shouldn\'t initialize vesting with zero token rewards", async () => {
            await expectRevert(
                EqiFi.new(identity.address, rewardToken.address, startDate, ether("0")),
    ...
    describe("EqiFI Distribution Withdraw Phase Test Cases", function () {
        it("should release tokens correct", async () => {
            totalAllocatedAmount = new BN(await rewardToken.balanceOf(EF));
            tokensForNative = totalAllocatedAmount.div(new BN(3));
            tokensForLP = totalAllocatedAmount.sub(tokensForNative);
            (await eqiFi.totalAllocatedAmount()).should.be.bignumber.equal(totalAllocatedAmount);
            (await eqiFi.tokensForNative()).should.be.bignumber.equal(tokensForNative);
            (await eqiFi.tokensForLP()).should.be.bignumber.equal(tokensForLP);
        });

        it('shouldn\'t withdraw rewards before start date', async () => {
            startDateNew = startDate + vestingDuration;
            rewardToken = await MockErc20.new(ether("3000"), { from: owner });
            eqiFi = await EqiFi.new(identity.address, rewardToken.address, startDateNew, ether("3000"));
            EF = eqiFi.address;
            await rewardToken.transfer(EF, ether("3000"));
        ...
        it('should withdraw rewards before vesting ends correctly', async () => {
            ...
            result = await eqiFi.withdrawReward(
                percentageLp,
                percentageNative,
                vrs.v,
                vrs.r,
            ...
            (await rewardToken.balanceOf(beneficiary1)).should.be.bignumber.at.most(ether("66.66667"));
            (await rewardToken.balanceOf(beneficiary1)).should.be.bignumber.at.least(ether("66.6666"));
        ...
        it('should withdraw rewards after vesting duration correctly', async () => {
            ...
            let signature = EthCrypto.sign(identity.privateKey, message);
            vrs = EthCrypto.vrs.fromString(signature);
            await timeMachine.advanceTime(aMonth * 7);
            result = await eqiFi.withdrawReward(
                percentageLp,
                percentageNative,
                vrs.v,
        ...
        it('should add new signer correctly', async () => {
            ...
            await eqiFi.changeSignerList(newSigner.address, true);
            let message = EthCrypto.hash.keccak256([
                { type: "address", value: beneficiary1 },
                { type: "uint256", value: percentageLp },
                { type: "uint256", value: percentageNative },
        ...
        it('shouldn\'t add signer with zero address', async () => {
            await expectRevert(
                eqiFi.changeSignerList(constants.ZERO_ADDRESS, true),
                "Invalid signer address"
            );
        });
        ...
        it('should withdraw any erc20 send accidentally to the contract', async () => {
            ...
            await erc20.transfer(EF, amount, { from: beneficiary1 });
            await eqiFi.withdrawERC20(erc20.address, amount);
        ...
        it('shouldn\'t withdraw tokens if insufficient tokens balance in contract', async () => {
            ...
            await erc20.transfer(EF, amount, { from: beneficiary1 });
            await expectRevert(
                eqiFi.withdrawERC20(erc20.address, ether("3000")),
