...
contract("RaiinMaker", function (accounts) {
    ...
    describe("RaiinMaker Distribution Initializing Phase Test Cases", function () {
        ...
        it("should initialize vesting with correct remaining percentage", async () => {
            (await raiinMaker.remainingPercentage()).should.be.bignumber.equal(remainingPercentage);
        });
        ...
        it("shouldn\'t initialize vesting with empty cliff/vesting duration", async () => {
            await expectRevert(
                RaiinMaker.new(identity.address, rewardToken.address, startDate, 0, vestingDuration, tgePercentage, ether("9000")),
        ...
        it("shouldn\'t initialize vesting with empty tge tpercentage", async () => {
            await expectRevert(
                RaiinMaker.new(identity.address, rewardToken.address, startDate, cliffDuration, vestingDuration, ether("0"), ether("9000")),
